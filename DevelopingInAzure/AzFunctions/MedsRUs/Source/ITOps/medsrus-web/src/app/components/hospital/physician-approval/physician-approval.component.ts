...
@Component({
  selector: 'app-physician-approval',
  templateUrl: './physician-approval.component.html',
  styleUrls: ['./physician-approval.component.css']
})
export class PhysicianApprovalComponent implements OnInit {
  ...
  onApproveRejectOrder(orderStatus: string): void {
    ...
    this.hospitalService.UpdateOrderById(this.medicineOrderApproval).subscribe(res => {
      ...
      this.ngZone.run(() => this.router.navigateByUrl('/physician'))
    });
